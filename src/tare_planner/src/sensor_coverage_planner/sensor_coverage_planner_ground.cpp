...
bool PlannerParameters::ReadParameters(ros::NodeHandle& nh)
{
  /**
   * 获取的话题：
   * /exploration_start, /state_estimation_at_scan, /registered_scan, /terrain_map, /terrain_map_ext, /coverage_boundary
   * /navigation_boundary, /nogo_boundary
   * 发布的话题：
...
void PlannerData::Initialize(ros::NodeHandle& nh, ros::NodeHandle& nh_p)
{
  ...
  moving_forward_ = true; //机器人直走标志默认为true
...
{
  initialize(nh, nh_p); 
  PrintExplorationStatus("Exploration Started", false); //打印探索程序状态
}
